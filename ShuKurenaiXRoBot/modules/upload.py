...
from ShuKurenaiXRoBot.modules.urluploader import download_file
from ShuKurenaiXRoBot.utils.uputils import humanbytes, progress
...
async def tmp(event):
    if event.reply_to_msg_id:
        ...
        try:
            orta = await event.respond("Uploading to ShuKurenaiXRoBot...")
            download_link = await send_to_tmp_async(file_path)

            str(time.time() - start)
            await orta.edit(
                f"File Successfully Uploaded to ShuKurenaiXRoBot.\n\nLink 👉 {download_link}\n\nUploaded by *ShuKurenaiXRoBot*"
            )
        except Exception as e:
            traceback.print_exc()
            print(e)
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            await bot.send_file(
                ...
                dosya,
                force_document=True,
                caption=f"Uploaded By *Kiara_ro_bot*",
            )
        except Exception as e:
            traceback.print_exc()
